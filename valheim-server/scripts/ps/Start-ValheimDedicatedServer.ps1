...
.EXAMPLE
    ...
    .\Start-ValheimDedicatedServer.ps1 -PathToSteamCMD C:\steamcmd -PathToValheimBackup C:\valheimbackup -PathToValheimInstall C:\valheim [-PathToData c:\valheim_data]
...
function backup_valheim_files () {
    ...
    if ($PathToValheimBackup) {
        ...
        if (-Not $PathToData) {
            ...
            $PathToData = $home + '\AppData\LocalLow\IronGate\Valheim';
        ...
        if ($TestIrongate -eq $true) {
            ...
            copy-item -path $Irongate -recurse -destination "$PathToValheimBackup\$Date";
        }

        else {
    
            write-host;
            write-host "Could not find the data folder containing your worlds. If this is not a fresh install, please check that the data folder exists within $Irongate." -foregroundcolor yellow;
            write-host;
...
function backup_headless_server() {
    ...
    if ($TestHeadlessServer -eq $true) {
        ...
        $null = copy-item -path "$PathToValheimInstall\start_headless_server.bat" -destination $PathToValheimBackup\$Date;
    }
...
try {
    ...
    ui_start_status "Backing up $PathToData directory to ""$PathToValheimBackup\$Date""";
